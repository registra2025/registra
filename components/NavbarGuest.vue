...
<template>
    <header  class="w-[calc(100%-0.5rem)] h-[73px] bg-white border-2 border-text flex items-center px-4 fixed top-1 left-1 right-1 z-50 rounded-[15px] shadow-lg">
        <button
        @click="emit('toggleSidebar')"
        class="text-[#2170d4] text-[32px] w-13 h-13 p-0.5 border-1 border-black rounded-md transition transform  active:scale-90 duration-100 ease-in-out"
        >
        <span class="material-symbols-outlined">menu</span>
    </button>
    <h1 class="text-text text-xl sm:text-2xl md:text-3xl text-light ml-4">REGISTRA</h1>
    <!-- Dark Mode Toggle -->
